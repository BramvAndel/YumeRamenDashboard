...
async function fetchMeals() {
    ...
    try {
        ...
        if (mealsList) {
            mealsList.innerHTML = '<p style="color: red; text-align: center;">Error loading meals.</p>';
        }
    }
}
...
function displayMeals(meals) {
    ...
    meals.forEach(meal => {
        ...
        const imageSrc = meal.Image ? SERVER_URL + meal.Image : 'https://via.placeholder.com/150?text=No+Image';
        ...
        card.innerHTML = `
            ...
            <div class="order-details" style="display: flex; gap: 15px; align-items: center;">
                ...
                <div>
                    ...
                    <p style="margin: 0; color: #7f8c8d; font-size: 0.9rem;">${meal.Ingredients || meal.Description || ''}</p>
                </div>
            </div>
...
function openMealModal(mealId = null) {
    ...
    if (mealId) {
        ...
        const meal = currentMeals.find(m => (m.DishID || m.MealID || m.id) == mealId);
        if (meal) {
            document.getElementById('meal-id').value = meal.DishID || meal.MealID || meal.id;
            document.getElementById('meal-name').value = meal.Name || meal.name || '';
            document.getElementById('meal-price').value = meal.Price;
            document.getElementById('meal-ingredients').value = meal.Ingredients || meal.Description || '';
...
async function deleteMeal(mealId) {
    ...
    try {
        const response = await authenticatedFetch(SERVER_URL + `api/v1/dishes/${mealId}`, {
            method: 'DELETE'
        });
