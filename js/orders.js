...
async function fetchOrders() {
    ...
    try {
        ...
        try {
            const userResponse = await authenticatedFetch(SERVER_URL + 'api/v1/users');
                ...
                if (Array.isArray(users)) {
                    users.forEach(u => {
                        ...
                        if (uid) {
                            ...
                            currentUserMap[uid] = `${firstName} ${lastName}`.trim() || 'Unknown';
                        }
                    });
                }
            }
...
function renderCurrentOrders() {
    ...
    // Update Badge
    const activeCount = orders.filter(order => (order.Status || order.status || 'ordered').toLowerCase() !== 'completed').length;
    ...
    const totalCompletedCount = orders.filter(order => (order.Status || order.status || '').toLowerCase() === 'completed').length;
    ...
    // 1. Get Filter Values
    const activeSort = document.getElementById('active-sort') ? document.getElementById('active-sort').value : 'date-desc';
    const activeStatus = document.getElementById('active-filter-status') ? document.getElementById('active-filter-status').value : 'all';
    const activePaid = document.getElementById('active-filter-paid') ? document.getElementById('active-filter-paid').value : 'all';
    const activeId = document.getElementById('active-filter-id') ? document.getElementById('active-filter-id').value.trim() : '';
    
    const completedSort = document.getElementById('completed-sort') ? document.getElementById('completed-sort').value : 'date-desc';
    const completedPaid = document.getElementById('completed-filter-paid') ? document.getElementById('completed-filter-paid').value : 'all';
    const completedId = document.getElementById('completed-filter-id') ? document.getElementById('completed-filter-id').value.trim() : '';

    // 2. Split Orders
    let activeOrders = orders.filter(o => (o.Status || o.status || 'ordered').toLowerCase() !== 'completed');
    let completedOrders = orders.filter(o => (o.Status || o.status || 'ordered').toLowerCase() === 'completed');

    // 3. Filter Active Orders
    if (activeId) {
        activeOrders = activeOrders.filter(o => o.OrderID.toString().includes(activeId));
    }
    if (activeStatus !== 'all') {
        activeOrders = activeOrders.filter(o => (o.Status || o.status || 'ordered').toLowerCase() === activeStatus);
    }
    if (activePaid !== 'all') {
    ...
    if (completedId) {
        completedOrders = completedOrders.filter(o => o.OrderID.toString().includes(completedId));
    }
    ...
    const sortOrders = (list, sortValue) => {
        return list.sort((a, b) => {
            if (sortValue === 'date-desc') return new Date(b.Ordered_at || 0) - new Date(a.Ordered_at || 0);
            if (sortValue === 'date-asc') return new Date(a.Ordered_at || 0) - new Date(b.Ordered_at || 0);
            if (sortValue === 'id-desc') return b.OrderID - a.OrderID;
            if (sortValue === 'id-asc') return a.OrderID - b.OrderID;
            if (sortValue === 'price-desc') return calculateOrderTotal(b) - calculateOrderTotal(a);
            if (sortValue === 'price-asc') return calculateOrderTotal(a) - calculateOrderTotal(b);
            return 0;
        });
    };
    ...
    const renderCard = (order, container) => {
        ...
        const date = order.Ordered_at ? new Date(order.Ordered_at).toLocaleString() : 'Date Unknown';
        ...
        card.innerHTML = `
            ...
            <div class="order-details">
                <p>Status: <strong>${status.toUpperCase()}</strong> ${isPaid ? '<span style="color:green; margin-left:10px">PAID</span>' : '<span style="color:red; margin-left:10px">UNPAID</span>'}</p>
...
async function showOrderDetails(orderId) {
    ...
    const userName = currentUserMap[order.UserID] || `User #${order.UserID}`;
    const dateOrdered = order.Ordered_at ? new Date(order.Ordered_at).toLocaleString() : 'Date Unknown';
    const dateProcessing = order.processing_at ? new Date(order.processing_at).toLocaleString() : '-';
    const dateDelivering = order.Delivering_at ? new Date(order.Delivering_at).toLocaleString() : '-';
    const dateCompleted = order.Completed_at ? new Date(order.Completed_at).toLocaleString() : '-';
        ...
        <div class="modal-info-section full-width">
            ...
            <div class="timeline-visual">
                ...
                <div class="timeline-steps">
                    <!-- Step 1: Ordered -->
                    <div class="timeline-step ${['ordered', 'processing', 'delivering', 'completed'].includes(order.Status || 'ordered') ? 'active' : ''}">
                    ...
                    <!-- Step 2: Processing -->
                    <div class="timeline-step ${['processing', 'delivering', 'completed'].includes(order.Status) ? 'active' : ''}">
                        <div class="step-dot"></div>
                        <span class="step-label">Processing</span>
                        <span class="step-time">${dateProcessing !== '-' ? dateProcessing : ''}</span>
                    </div>

                    <!-- Step 3: Delivering -->
                    <div class="timeline-step ${['delivering', 'completed'].includes(order.Status) ? 'active' : ''}">
                        <div class="step-dot"></div>
                        <span class="step-label">Delivering</span>
                        <span class="step-time">${dateDelivering !== '-' ? dateDelivering : ''}</span>
                    </div>
                    ...
                    <div class="timeline-step ${order.Status === 'completed' ? 'active' : ''}">
                        ...
                        <span class="step-time">${dateCompleted !== '-' ? dateCompleted : ''}</span>
                    </div>
                </div>
            </div>
        </div>
